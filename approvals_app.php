...
//*********************************************************************************SECOND TABLE WITH THE APPROVED DOCUMENTS**************************************************************************
	session_start();
...
//QUERY FOR THE APPROVE MAIN FORM -->
	...
	$query=	"SELECT UD.ID, YR, NO, DT.TYPE_ID, DOC_ID, DD.DOC_DESCR, " .
			"CASE WHEN UD.DOC_DESCR='' THEN DESCR ELSE UD.DOC_DESCR END AS DESCR, " . 
			"PRINT, concat(U.L_NAME, ' ' , U.F_NAME, ' at ', DATE_FORMAT(U_DT,'%d.%m.%y %H:%i')) AS U_FLNAME_DT, " . 
			"concat(A.L_NAME, ' ', A.F_NAME, ' at ', DATE_FORMAT(GA_DT,'%d.%m.%y %H:%i')) AS A_FLNAME_DT, A_USR, GA_USR " . 
			...
			//"AND UD.ACTIVE=1 "; Csak a munkaallomasoknal kell az aktivakra szurni
...
//SORT THE DATA -->
	...
	{
		$query = $query . " ORDER BY GA_DT DESC ";
	}
	...
	while($row=mysqli_fetch_array($result,MYSQLI_ASSOC))
	{
		...
		$approvals_app[] = 	array
							(
								...
								'A_FLNAME_DT' => $row['A_FLNAME_DT'],
								'U_LNK_V' => $v_lnk
							);
	}
	
	echo json_encode($approvals_app);
