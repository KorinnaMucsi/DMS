	...
	if(isset($_POST['submit']))
	{
		...
		while($row_sel_ctrl=mysqli_fetch_array($result_sel_ctrl))
		{
			...
			else
			{
				...
				$query_select="SELECT HR_FUNC_ID FROM tUsrRoles WHERE USR_ID='" . $_POST['selUsr'] . "' AND FROM_HR=0";
				...
				$query_delete="DELETE FROM tUsrRoles WHERE USR_ID='" . $_POST['selUsr'] . "' AND FROM_HR=0 ";
				mysqli_query($conn,$query_delete);
				
//HISTORY - DELETE
				History('tUsrRoles', '', 'USR_ID: ' . $_POST['selUsr'] . ', HR_FUNC_ID: ' . $hr_func_id, '', 'D', 'Temporary job title deleted from the user (if existed)', 
...
//FINAL MESSAGE				
	...
	$ws_users_form='<div id="mainContainerDiv" class="main_mainContainerDiv">' . "\n" .
					...
					'<span class="main_subtitle">You can find the list of the users with the temporary job titles below.<br>' ."\n" .
					...
					$msg . "\n" .	
					'<button type="button" id="btnUsrRoles" name="btnUsrRoles" onclick="UsrRoleVisible();">Add new temprary job title to user</button>' . "\n" .	
					'<form action="main.php?showWSUsers=True" method="POST" id="form_wsu">' . "\n" .
							'<div class="main_dataDiv" id="main_dataDiv_a">' . "\n" .
								...
								'<button type="button" id="btnFnsCancel" name="btnFnsCancel" onclick="UsrRoleInVisible()";>Cancel</button>' . "\n" .
							'</div>' . "\n" .
					'</form>' . "\n" .
					'<span class="main_subtitle"><i><b>Users with temporary job titles:</b></i></span>' . "\n" .
